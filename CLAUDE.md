...
- Tests: pytest, verified baseline 2629 passing tests
- Verified release base: 624673d — v0.3.4 merge (PR #52)
- Latest numbered release: v0.3.4 — MLB Walk-Forward Evaluation & Calibration Measurement (PR #52, merge commit 624673d, implementation commit 9ec05cd)
- Current roadmap: no next version is approved or assigned. The immediate technical next step is empirical, not a new version — run the now-frozen v0.3.4 evaluator against real historical MLB data and inspect the result. Do not invent, pre-approve, or begin a v0.3.5 scope. Any further version is scoped only after that empirical evaluation is inspected, and still requires its own inspection/planning stage and ChatGPT approval before coding begins.
...
- MLB now has a v0.3.1 pitch-level Statcast historical data foundation, a v0.3.2 plate-appearance dataset/rate foundation built on top of it, a v0.3.3 plate-appearance probability baseline built on top of that, and a v0.3.4 walk-forward evaluation and calibration-measurement layer that scores those probabilities against realized outcomes. v0.3.4 establishes the evaluation capability only — it measures, it never adjusts a probability or selects a configuration, and it has not yet been run on a real historical dataset, so RSB has no empirical evidence that any MLB method or hyperparameter configuration is superior. MLB still has no simulation, no PA-opportunity/lineup model, no prediction-time (upcoming-PA) input contract, no sportsbook bridge, and no operational MLB runtime/orchestrator. The MLB engine is not live-ready.
...
- See docs/MLB_NBA_ROADMAP.md for the full roadmap, including the named MLB architectural gaps that remain unscheduled.
...
Completed foundation (v0.1.8.x – v0.3.4):
...
- v0.3.3: MLB Plate-Appearance Probability Baseline — turns v0.3.2's leakage-safe prior batter/pitcher/league outcome counts into one coherent categorical probability distribution over the 12 `RATE_CATEGORIES` (src/mlb/plate_appearance_probability.py; docs/MLB_PLATE_APPEARANCE_PROBABILITY_CONTRACT.md). Four methods (`league_only`, `batter_shrinkage`, `pitcher_shrinkage`, `matchup_combination`): a league categorical baseline smoothed toward uniform, batter and pitcher Dirichlet shrinkage toward that league baseline, and a multiplicative batter/pitcher/league matchup baseline (log5-equivalent in the binary case) computed in log space. `intentional_walk` is preserved as its own outcome, never merged into `walk`. Strict positivity and normalization are enforced and never silently renormalized; leakage is prevented by a field-set whitelist that withholds the realized PA outcome and status, and pitcher-dependent methods fail closed (`PitcherAttributionUnavailableError`) when `pitcher_rate_eligible = False`, with the terminal `pitcher_id` withheld from those records. Model hyperparameters are provisional and explicitly not empirically optimized (league prior strength 1.0, batter 100.0, pitcher 100.0); every output record carries the strengths actually used plus `model_config_version` so artifacts can never be silently incomparable. Merged to main (PR #50, merge commit dcab8bb, implementation commit 72c4a8f, 73 new probability tests, 2380 passing tests). Feature branch feature/v0.3.3-mlb-pa-probability-baseline deleted locally and remotely. v0.3.3 itself added no walk-forward evaluation or calibration; that measurement capability was subsequently delivered by v0.3.4. Everything else v0.3.3 excluded remains unscheduled and is not assigned to v0.3.4 or to any specific later version: probability persistence, handedness/platoon splits, PA-opportunity/lineup/game-sequencing modeling, a prediction-time (upcoming-PA) input contract, a PA-start-pitcher prior contract, simulation, and sportsbook/EV/runtime wiring. See docs/MLB_NBA_ROADMAP.md §11 for those named architectural gaps.
- v0.3.4: MLB Walk-Forward Evaluation & Calibration Measurement — measures how well v0.3.3's per-plate-appearance probabilities score and calibrate against realized outcomes (src/calibration.py, src/mlb/plate_appearance_evaluation.py; docs/MLB_PLATE_APPEARANCE_EVALUATION_CONTRACT.md). Generic, sport-agnostic calibration primitives (reliability-bin construction, expected and maximum calibration error) live in src/calibration.py. The MLB evaluator adds multiclass log loss and Brier score as the two primary comparison metrics, descriptive accuracy, top-label reliability/ECE/MCE, 12 classwise one-vs-rest calibration diagnostics plus an unweighted macro classwise ECE summary, intersection-only comparative sampling so every compared configuration shares one denominator, separate method-support/coverage reporting (never folded into a metric), optional tuning/holdout date-split support, calendar-month diagnostics exposing cold start with no burn-in filter, deterministic input/config/report identity (content-derived input_content_sha256 and config_id, no timestamp anywhere), prior-state coherence validation against attach_prior_outcome_rates, and a strict self-consistency report validator. Only pa_status = "completed" plate appearances are scorable; walk-forward safety is inherited from v0.3.2 rather than reimplemented; bin edges and sample basis are fixed constants, not caller arguments. Merged to main (PR #52, merge commit 624673d, implementation commit 9ec05cd, 249 new tests — 73 calibration, 176 evaluation — 2629 passing tests). Feature branch feature/v0.3.4-mlb-walk-forward-evaluation-calibration deleted locally and remotely. v0.3.4 is a measurement layer only: no recalibration or probability adjustment, no automatic tuning or hyperparameter selection, no change to v0.3.3 defaults or PROBABILITY_MODEL_CONFIG_VERSION, no persistence, no database access, no simulation, no sportsbook/EV/ranking, and no MLB runtime orchestration. Critically, shipping the capability is not evidence: no real historical dataset has been evaluated, so no MLB method or configuration has been empirically selected. See docs/MLB_PLATE_APPEARANCE_EVALUATION_CONTRACT.md §16-§17 for the full non-goals list and the items recorded but not implemented.

...
- Added `src/mlb/`, a new package establishing a trustworthy, RSB-owned, provenance-aware historical MLB Statcast data foundation from manually supplied Baseball Savant CSV exports. RSB performs no automated interaction with MLB Digital Properties in this version — the operator supplies a locally downloaded CSV export and its local path; no HTTP requests, scraping, browser automation, or provider library (e.g. PyBaseball) is used. No legal-compliance claim is made about the operator's data use.
  - `statcast_import.py`: `load_statcast_export(path, *, declared_query, requested_date_range, source_downloaded_at=None, now_fn=None) -> StatcastExport`. Reads a local CSV export and validates file existence/readability/UTF-8/header structure. The adapter recognizes 33 mapped provider columns (`EXPECTED_SOURCE_COLUMNS`); mandatory normalized fields require their backing source headers (`REQUIRED_SOURCE_COLUMNS`, 17 columns, derived from `statcast_normalize.REQUIRED_NORMALIZED_FIELDS`), while the other 16 provider-nullable mapped columns may be entirely absent from the header and normalize to `null`, same as a present-but-blank cell — extra unmapped columns are also tolerated. Preserves raw bytes and source row order exactly. No baseball-semantic normalization. `StatcastImportError(ValueError)`. `declared_query` is a structured, JSON-serializable `dict` (string keys; non-serializable values rejected, not silently converted) preserving whatever search/filter controls the operator declares — not free-form prose. Does not infer `source_downloaded_at` from filesystem metadata — it is only ever operator-supplied, and when supplied must include an explicit UTC offset or `Z` (naive datetimes rejected) and is normalized to a canonical `...Z` UTC string. `ingestion_started_at`/`ingestion_completed_at` are RSB's own clock, timezone-aware UTC with microsecond precision (test-injectable via `now_fn`).
  - `statcast_normalize.py`: `normalize_statcast_row()` / `normalize_statcast_snapshot()` build the RSB-owned normalized pitch-level contract from the Stage-1-approved source field mapping (`SOURCE_FIELD_MAPPING`). Fail-closed: a missing/malformed required identity, chronology, game-context, or pre-pitch-state field raises `StatcastNormalizationError` and fails the whole snapshot; legitimately provider-nullable fields (baserunners, pitch/batted-ball detail, `pa_event_raw`) normalize to `None`. `normalize_statcast_snapshot()` validates `snapshot_id`/`source_provider`/`requested_date_range` before iterating any rows, so these are enforced even for a zero-row snapshot. Baseball sanity bounds enforced: `balls` 0–3, `strikes` 0–2, `outs` 0–2, `inning >= 1`. `rsb_pitch_id` is a deterministic SHA-256 of `(source_game_id, at_bat_number, pitch_number)`, validated unique within every snapshot. `pa_event_raw` is a direct, unvalidated passthrough of the provider's `events` column — no `pa_terminal`/terminal-PA taxonomy derivation (that is v0.3.2 scope). No `venue` field.
  - `statcast_snapshot.py`: `create_statcast_snapshot(path, *, declared_query, requested_date_range, source_downloaded_at=None, now_fn=None) -> dict` orchestrates import → normalize → persist. `source_provider` is fixed to `SOURCE_PROVIDER = "baseball_savant"` — not a caller-overridable argument — so the manifest and every normalized row always agree on `source_provider` by construction. Writes three immutable artifacts per snapshot: raw `.csv.gz` (gzip of the exact original uncompressed CSV bytes), normalized `.jsonl.gz` (gzip of canonical, deterministically ordered/serialized JSONL — one compact `json.dumps(..., separators=(",", ":"))` object per line, ordered by `(source_game_id, at_bat_number, pitch_number)`), and a `.manifest.json` with `snapshot_id`, `source_provider`, `retrieval_mechanism` (`"manual_csv_export"`), `source_identifier`, `declared_query`, `requested_date_range`, `source_downloaded_at`, `ingestion_started_at`, `ingestion_completed_at`, `adapter_version`, `normalized_schema_version`, `raw_content_sha256`, `normalized_content_sha256`, `raw_row_count`, `normalized_row_count`. `raw_content_sha256`/`normalized_content_sha256` are SHA-256 over the exact uncompressed bytes, computed before gzip. `snapshot_id` is `<ingestion_timestamp, microsecond-precision>_<raw_sha256_prefix12>` — content-and-ingestion-event-aware: re-ingesting identical raw bytes later yields the same raw hash but a different `snapshot_id` (microsecond-resolution, so two ingestions within the same second still differ) and therefore a different `normalized_content_sha256` (snapshot_id is embedded per row). Existing snapshot artifacts are never overwritten (`StatcastSnapshotError`); overlapping/duplicate re-ingestions coexist as separate snapshots.
- Extended `src/paths.py` with an MLB data root: `DATA_MLB_DIR`, `get_mlb_data_dir()` (overridable via `RSB_MLB_DATA_DIR`, following the existing `RSB_*_PATH` override convention), and `get_mlb_manual_input_dir()` / `get_mlb_raw_statcast_dir()` / `get_mlb_normalized_statcast_pitch_dir()` / `get_mlb_snapshot_dir()` for the `data/mlb/{manual_input,raw/statcast,normalized/statcast_pitch,snapshots}` layout. `data/mlb/` added to `.gitignore`; no real historical MLB data is committed.
- Added 218 new tests (`tests/test_statcast_import.py`, `tests/test_statcast_normalize.py`, `tests/test_statcast_snapshot.py`, plus MLB path-override tests in `tests/test_paths.py`) and a tiny synthetic fixture (`tests/fixtures/mlb/statcast_sample.csv`), covering structural/header/encoding validation, required-vs-optional header and field behavior (including missing optional header columns normalizing to `null`), baseball sanity bounds, structured `declared_query` validation, snapshot-level argument validation on zero-row snapshots, provider consistency, microsecond timestamp precision, `source_downloaded_at` timezone handling, deterministic serialization/checksums, snapshot immutability and coexistence, and AST-based architecture isolation (new MLB modules do not import `run_slate`, `database`, `data_quality`, or `market_selector`). Full suite: 2156 passed (1938 baseline + 218 new), 0 failures, 0 skips.
- Zero new dependencies. CSV parsing uses the Python standard library (`csv`) rather than pandas.
- No plate-appearance aggregation, terminal-PA taxonomy, batter/pitcher rates, league priors, feature engineering, probability generation, modeling, calibration, simulation, sportsbook odds, candidate EV/ranking, or MLB runtime wiring in this version — that begins at v0.3.2. `data/worldcup_ai.db`, `src/database.py`, and the World Cup runtime (`run_slate.py` and everything it orchestrates) are unchanged.
